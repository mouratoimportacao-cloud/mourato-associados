...
export default function ImportPreviewModal({ base64Data, onClose, onSuccess }: ImportPreviewModalProps) {
  ...
  const [activeTab, setActiveTab] = useState<"update" | "error">("update");
  ...
  useEffect(() => {
    startAnalysis(async () => {
      ...
      if (res.success) {
        ...
        if (res.stats) {
          if (res.stats.update > 0) setActiveTab("update");
  ...
  const handleConfirmImport = () => {
    ...
    startImport(async () => {
      try {
        const res = await executarImportacaoAction(base64Data);
        if (res.success) {
          setFinalReport(res);
          setStep("result");
        } else {
      ...
      <div className="bg-neutral-950 border border-zinc-800 rounded-3xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden text-zinc-100">
        ...
        <header className="p-6 border-b border-zinc-900 flex justify-between items-center bg-zinc-900/30">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1 uppercase tracking-wider">
              {step === "preview" && "Etapa 1: Validar produtos e prévia de alteração"}
              {step === "result" && "Etapa 2: Relatório final da importação"}
            </p>
          </div>
          {step !== "result" && (
            <button
        ...
        {!errorMessage && isAnalyzing && step !== "result" && (
          <div className="flex-grow flex flex-col items-center justify-center p-12 space-y-4 bg-zinc-950/80">
            ...
            <p className="text-sm text-zinc-400 font-medium animate-pulse">Processando dados e cruzando com o catálogo...</p>
          </div>
        )}
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-zinc-900/30 border border-zinc-900 p-5 rounded-2xl">
                    ...
                    <span className="text-[10px] text-zinc-400 mt-1 block">Produtos identificados no catálogo</span>
                  </div>
                  <div className="bg-zinc-900/30 border border-zinc-900 p-5 rounded-2xl">
                  ...
                  <div className="bg-zinc-900/30 border border-zinc-900 p-5 rounded-2xl">
                    ...
                    <span className="text-[10px] text-zinc-400 mt-1 block">Linhas ignoradas (devem ser cadastradas à mão)</span>
                  </div>
                </div>
                                  ...
                                  <div className="space-y-1.5">
                                    {Object.keys(row.diff).length === 0 ? (
                                      ...
                                      Object.entries(row.diff).map(([key, value]: [string, any]) => {
                                        const label = 
                                          key === "precoSugerido" ? "Preço Sugerido / Venda" :
                                          key === "custoDolar" ? "Custo USD" :
                                          key === "cotacaoDolar" ? "Cotação USD" :
                                          key === "estoqueGeral" ? "Estoque Geral" : key;
                                        const isMoney = key === "precoSugerido" || key === "custoDolar";
            ...
            {step === "result" && finalReport && (
              <div className="space-y-6 text-center py-6">
                ...
                <div className="max-w-md mx-auto bg-zinc-900/30 border border-zinc-900 rounded-3xl p-6 grid grid-cols-2 gap-4 text-center">
                  <div>
        ...
        {/* MODAL FOOTER */}
        <footer className="p-6 border-t border-zinc-900 bg-zinc-900/10 flex justify-end items-center">
