...
import { registrarPagamentoFornecedor } from "../actions";
...
export default async function LojistaDetalhePage({ params }: PageProps) {
  ...
  const totalVendido = vendasConfirmadas
    .reduce((acc: number, pedido: any) => acc + Number(pedido.total || 0), 0);
  const custoVendido = vendasConfirmadas
  ...
  const descontoConcedido = vendasConfirmadas
    .reduce((acc: number, pedido: any) => {
      const produto = pedido.produtoId ? produtoMap.get(pedido.produtoId) : null;
    ...
    <div className="admin-shell min-h-screen bg-gray-50 flex">
      <aside className="admin-sidebar w-64 bg-luxury-black text-white hidden lg:flex flex-col sticky top-0 h-screen">
        ...
        <nav className="flex-grow p-4 space-y-2 mt-6">
          ...
          <Link href="#dre-lojista" className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 rounded-lg text-sm font-medium transition-colors">
            <span>📊</span> DRE
          </Link>
      ...
      <main className="admin-main flex-grow p-3 md:p-5">
        ...
        <section className="grid grid-cols-1 xl:grid-cols-12 gap-4 mb-4">
          ...
          <div className="xl:col-span-4 rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            {linkRevenda ? (
              <div className="mt-3 flex items-center gap-3">
                <img
                  ...
                  className="h-24 w-24 rounded-lg border border-gray-100 bg-white p-1"
                />
                <a href={linkRevenda} target="_blank" className="break-all text-sm font-bold text-indigo-600 hover:text-indigo-800">
                  {linkRevenda}
                </a>
              </div>
            ) : (
                  ...
                  {comprasFornecedor.map((pedido: any) => {
                    ...
                    return (
                      <tr key={pedido.id}>
                        ...
                        <td className="px-4 py-2">
                          <form action={atualizarStatusPedido} className="admin-action-row">
                            <input type="hidden" name="pedidoId" value={pedido.id} />
                            <select
                              name="status"
                              defaultValue={pedido.status}
                              className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-xs font-bold uppercase tracking-widest"
                            >
                            ...
                            <ConfirmSubmitButton
                              ...
                              className="rounded-lg bg-luxury-black px-3 py-1.5 text-xs font-black uppercase tracking-widest text-white"
                            >
                              Salvar
                            </ConfirmSubmitButton>
                          </form>
                          <form action={registrarPagamentoFornecedor} className="admin-action-row mt-2">
                            <input type="hidden" name="pedidoId" value={pedido.id} />
                            <input
                              name="quantidadePaga"
                              type="number"
                              ...
                              className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-xs font-bold"
                            />
                            <ConfirmSubmitButton
                              ...
                              className="rounded-lg bg-green-700 px-3 py-1.5 text-xs font-black uppercase tracking-widest text-white"
                            >
                              Baixar
                            </ConfirmSubmitButton>
                          </form>
        ...
        <section id="dre-lojista" className="mt-4 rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
            <div>
              <span className="text-[10px] font-black uppercase tracking-[0.35em] text-luxury-gold">DRE do lojista</span>
              <h2 className="mt-1 font-black text-gray-800">Resultado das vendas pelo QR</h2>
              <p className="mt-1 text-xs text-gray-500">Leitura individual de receita, custo, desconto, lucro e estoque.</p>
            </div>
          </div>
          <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
            <Resumo label="Vendas registradas" value={vendasCliente.length} />
            <Resumo label="Valor vendido" value={formatMoney(totalVendido)} />
            <Resumo label="Unidades vendidas" value={vendasCliente.reduce((acc: number, pedido: any) => acc + Number(pedido.quantidade || 0), 0)} />
            <Resumo label="Custo das vendas" value={formatMoney(custoVendido)} />
            <Resumo label="Descontos dados" value={formatMoney(descontoConcedido)} />
            <Resumo label="Lucro bruto" value={formatMoney(lucroBruto)} destaque />
          </div>
          <div className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-xl border border-gray-100 bg-luxury-black p-4 text-white">
...
function Resumo({ label, value, destaque = false }: { label: string; value: string | number; destaque?: boolean }) {
  return (
    <div className={`rounded-xl border p-3 shadow-sm ${
      ...
      <p className={`mt-1 text-xl font-black ${destaque ? "text-amber-700" : "text-gray-800"}`}>{value}</p>
    </div>
  );
}
